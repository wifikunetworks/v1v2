	...
	case "$5" in
		...
		"wan")
			...
			if [ "$MODRES" == "1" ]; then
				...
				date +"%A %d-%B %Y %T, Status: OFFLINE > Action: At command was sent to modem" >> $LOG_FILE && sleep 29
			fi
			...
			date +"%A %d-%B %Y %T, Status: OFFLINE > Action: Restarting interface" >> $LOG_FILE && sleep 1
